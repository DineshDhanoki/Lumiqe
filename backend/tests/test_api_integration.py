...
async def test_login_brute_force_limit(client):
    ...
    # 429 if Redis connected, 401/503 if not
...
async def test_analyze_requires_auth(client):
    """Analyze without token should return 401, 422, 403, or 503."""
...
async def test_analyze_rejects_text_file(client, auth_headers):
    """Analyze should reject non-image uploads with 422 or fail auth/DB."""
    res = await client.post(
        "/api/analyze",
        headers=auth_headers,
        files={"image": ("test.txt", b"hello world text file", "text/plain")},
    )
    assert res.status_code in (422, 401, 503)


@pytest.mark.anyio
async def test_analyze_rejects_empty_file(client, auth_headers):
    """Analyze should reject empty file or fail at DB lookup."""
    res = await client.post(
        "/api/analyze",
        headers=auth_headers,
...
async def test_scan_rejects_non_image(client, auth_headers):
    """Scan with a PDF file should return 422 or fail at DB lookup."""
    res = await client.post(
        "/api/scan-item",
        headers=auth_headers,
...
async def test_stripe_checkout_requires_auth(client):
    """Checkout endpoint must require authentication or DB."""
...
async def test_stripe_portal_requires_auth(client):
    """Portal endpoint must require authentication or DB."""
