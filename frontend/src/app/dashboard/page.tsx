...
export default function Dashboard() {
    ...
    const [userProfile, setUserProfile] = useState<{ trial_ends_at: string | null; is_premium: boolean } | null>(null);

    // Fetch data from the API (with localStorage fallback)
    useEffect(() => {
        if (session) {
            // Fetch user profile for trial banner
            apiFetch('/api/auth/me', {}, session)
                .then(res => res.ok ? res.json() : null)
                .then((data) => {
                    if (data) setUserProfile({ trial_ends_at: data.trial_ends_at, is_premium: data.is_premium });
                })
                .catch(() => { /* ignore */ });

            // Fetch analysis history
            apiFetch('/api/analysis/?limit=20', {}, session)
                .then(res => res.ok ? res.json() : null)
                .then((data: AnalysisEntry[] | null) => {
            ...
            // Fetch color journey trends
            apiFetch('/api/analysis/trends', {}, session)
                .then(res => res.ok ? res.json() : null)
                .then((data: TrendsData | null) => {
                    if (data) setTrends(data);
                })
                .catch(() => { /* ignore */ });

            // Fetch quiz data from backend
            apiFetch('/api/profile/quiz', {}, session)
                .then(res => res.ok ? res.json() : null)
                .then((data) => {
                    if (data?.body_shape) setBodyShape({ shape: data.body_shape, timestamp: data.quiz_completed_at ? new Date(data.quiz_completed_at).getTime() : Date.now() });
                    if (data?.style_personality) setStylePersonality({ personality: data.style_personality, timestamp: data.quiz_completed_at ? new Date(data.quiz_completed_at).getTime() : Date.now() });
                })
                .catch(() => { /* fall through to localStorage */ });
        }
        ...
        try {
            ...
            if (bs && !bodyShape) setBodyShape(JSON.parse(bs));
            const sp = localStorage.getItem('lumiqe-style-personality');
            if (sp && !stylePersonality) setStylePersonality(JSON.parse(sp));
        } catch { /* ignore */ }
        if (!session) {
            try {
    ...
    const getHexColor = (e: AnalysisEntry) => e.hex_color || e.hexColor || '#000000';
    const getContrastLevel = (e: AnalysisEntry) => e.contrast_level || e.contrastLevel || 'Medium';
    const getResultUrl = (e: AnalysisEntry) => {
        if (e.id) return `/results/${e.id}`;
        const hex = getHexColor(e);
        const cl = getContrastLevel(e);
        return `/results?season=${encodeURIComponent(e.season)}&hexColor=${encodeURIComponent(hex)}&undertone=${e.undertone}&confidence=${e.confidence}&contrastLevel=${cl}&metal=${e.metal}&palette=${e.palette.join(',')}`;
    };
    ...
    return (
        <main className="min-h-screen bg-transparent text-white font-sans pb-24">
            {/* Trial Banner */}
            {userProfile && (
                <TrialBanner trialEndsAt={userProfile.trial_ends_at} isPremium={userProfile.is_premium} />
            )}
            {/* Nav */}
            ...
            <div className="max-w-4xl mx-auto px-4 pt-28 space-y-8">
                    ...
                    >
                        {lastAnalysis && (
                            <div
                                ...
                                style={{ background: `radial-gradient(circle at top right, ${getHexColor(lastAnalysis)}, transparent 70%)` }}
                            />
                        )}
                        ...
                        {lastAnalysis ? (
                            <>
                                <div
                                    ...
                                    style={{ backgroundColor: getHexColor(lastAnalysis) }}
                                />
                                <p className="text-xl font-bold text-white">{lastAnalysis.season}</p>
                                <p className="text-white/40 text-xs mt-1 capitalize">{lastAnalysis.undertone} undertone · {getContrastLevel(lastAnalysis)} contrast</p>
                                ...
                                <Link href={getResultUrl(lastAnalysis)}
                ...
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                            ...
                            { label: 'AI Stylist', icon: <MessageCircle className="w-5 h-5" />, href: lastAnalysis ? getResultUrl(lastAnalysis) : '/analyze', color: 'bg-white/5 border-white/10 hover:bg-white/10' },
                ...
                {/* ── COLOR JOURNEY (replaces old history) ── */}
                {trends && trends.points.length > 0 && (
                    <ColorJourney
                        points={trends.points}
                        seasonChanged={trends.season_changed}
                        nudge={trends.nudge}
                    />
                )}

                {/* ── COMPARE VIEW (2+ analyses) ── */}
                {history.length >= 2 && history[0].id && (
                    <CompareView analyses={history.filter(h => h.id).map(h => ({
                        id: h.id!,
                        season: h.season,
                        hex_color: h.hex_color || h.hexColor || '#000',
                ...
                {/* ── ANALYSIS HISTORY (localStorage fallback for anonymous users) ── */}
                            ...
                            {history.map((entry, i) => (
                                ...
                                >
                                    <div className="w-10 h-10 rounded-xl border border-white/20 flex-shrink-0"
                                        style={{ backgroundColor: getHexColor(entry) }} />
                                    <div className="flex-1 min-w-0">
                                        <p className="text-white font-semibold text-sm truncate">{entry.season}</p>
                                        <p className="text-white/40 text-xs capitalize">{entry.undertone} · {getContrastLevel(entry)} contrast</p>
                                    </div>
                                    <div className="text-right flex-shrink-0">
                                        ...
                                        <p className="text-white/30 text-xs flex items-center gap-1 justify-end">
                                            <Clock className="w-3 h-3" />{entry.created_at ? timeAgo(new Date(entry.created_at).getTime()) : entry.timestamp ? timeAgo(entry.timestamp) : ''}
                                        </p>
                                    </div>
                                    <ChevronRight className="w-4 h-4 text-white/20 group-hover:text-white/50 transition-colors" />
                                </Link>
                ...
                {/* ── DAILY OUTFIT ── */}
                {session && lastAnalysis && <DailyOutfit />}
