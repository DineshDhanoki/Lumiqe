...
export default function AnalysisResultPage() {
    ...
    // Fetch the persisted analysis result
    useEffect(() => {
    ...
    // Fetch complete profile on tab switch
    useEffect(() => {
        if (activeTab !== 'overview' && analysis && !completeProfile && !profileLoading) {
            setProfileLoading(true);
            const baseSeason = analysis.season.replace(' (Neutral Flow)', '');
    ...
    const { season, hex_color: hexColor, undertone, confidence, contrast_level: contrastLevel, palette, avoid_colors: avoidColors, metal, full_result } = analysis;
    const description = full_result.description || '';
    const celebrities = full_result.celebrities || [];
    const makeup = full_result.makeup || { lips: '', blush: '', eyeshadow: '' };
    const tips = full_result.tips || '';
                    ...
                    >
                        ...
                        {activeTab === 'overview' && (
                            <div className="space-y-8">
                                ...
                                <ShareButtons analysisId={analysis.id} season={season} session={session} />
